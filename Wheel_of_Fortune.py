...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self,cetagory,obscurred_Phrase,guessed):
        print(
            """{} has ${}
...
""".format(self.name,self.prizeMoney,cetagory,obscurred_Phrase,', '.join(sorted(guessed))))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        if rand1 > self.difficulty:
            return True
        return False
    def getPossibleLetters(self,guessed):
        LETTERS1 = [letter for letter in LETTERS]
        for x in guessed:
            try:
                LETTERS1.remove(x)
            except:
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        p_l = self.getPossibleLetters(guessed)
        if p_l == []:
            return 'pass'
        move = self.smartCoinFlip()
        if move == True:
            x = len(self.SORTED_FREQUENCIES)
            while(x != 0):
                if self.SORTED_FREQUENCIES[x-1] in p_l:
                    return self.SORTED_FREQUENCIES[x-1]
                x -= 1
        else:
            return random.choice(p_l)
